...
def build_parser() -> argparse.ArgumentParser:
    ...
    snapshot.add_argument(
        "--redact", action="store_true", help="Pseudonymize sensitive fields before writing"
    )
    ...
    prune = commands.add_parser("prune", help="Plan or apply snapshot retention")
    prune.add_argument("--directory", type=Path, default=Path("snapshots"))
    prune.add_argument("--keep", type=int, default=100)
    prune.add_argument("--max-age-days", type=int, default=30)
    prune.add_argument("--apply", action="store_true", help="Delete planned files")
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "snapshot":
        ...
        if args.redact:
            ...
            if not key:
                raise SystemExit("Set NFR_REDACTION_KEY before using --redact")
            snapshot = redact_snapshot(snapshot, key)
        _write(args.output, json.dumps(snapshot, indent=2) + "\n")
        print(f"Snapshot written to {args.output}")
        return 0

    if args.command == "prune":
        candidates = prune_candidates(args.directory, args.keep, args.max_age_days)
        for path in candidates:
            print(path)
        if not args.apply:
            print(f"Dry run: {len(candidates)} file(s) would be removed; add --apply to delete")
            return 0
        removed = apply_prune(candidates)
        print(f"Removed {removed} snapshot file(s)")
        return 0
